...
#pragma endregion
	...
	cout << "Taille de la liste après ajout: " << (listeHeros.size() == heros.size() + 1) << " (" << listeHeros.size() << " ," << heros.size() << ")" << endl;
	...
	vector<Heros> herosOrdonnes;
	for (const auto &hero : listeHeros)
	{
		herosOrdonnes.push_back(hero);
	}
	std::sort(herosOrdonnes.begin(), herosOrdonnes.end());
	cout << separateurSections << "Héros en ordre alphabétique:" << endl;
	cout << "\033[94m"; // Réinitialise la couleur, pour que les couleurs différentes des héros soient plus visibles dans la liste liée."
	for (const auto &hero : herosOrdonnes)
	{
		hero.afficher(cout);
		cout << endl ;
	}
